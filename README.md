...
> Поточна версія: **1.1.0** (`python tender_scraper.py --version`, історія — у `CHANGELOG.md`)

Щоденний моніторинг публічних тендерів: офіційна база ЄС (TED) + bund.de + SIMAP (CH) +
USP Bekanntmachungsportal (AT, включно з підпороговими) + DAB.de + земельні портали DE.
Результат: CSV з відфільтрованими тендерами + Telegram-сповіщення з топ-10,
health-моніторингом джерел і (опційно) AI-аналізом релевантності.
...
### Level 2 — Claude API (ANTHROPIC_API_KEY) — вбудовано в дайджест
Додай у `.env`:
```
ANTHROPIC_API_KEY=sk-ant-...
```
Після цього `tender_digest.py` автоматично:
- додає до кожного топ-тендера в Telegram рядок «💡 чому це релевантно» (українською);
- генерує чернетки outreach-листів німецькою → `output/tender_outreach_<регіон>_<дата>.md`.

Модель: `claude-haiku-4-5` (швидко й дешево для коротких анотацій; змінюється через
`TENDER_AI_MODEL` у `.env`). Вимкнути разово: `--no-ai`.

### Level 3 — OpenAI GPT (OPENAI_API_KEY) — вбудований fallback
Альтернатива для клієнтів без доступу до Claude. Якщо `ANTHROPIC_API_KEY` нема, але є:
```
OPENAI_API_KEY=sk-...
```
дайджест використає OpenAI (модель `gpt-5.4-mini`, змінюється через `OPENAI_MODEL`).
...
| `--no-telegram` | — | Без відправки (прев'ю в консоль) |
| `--no-ai` | — | Без AI-аналізу навіть з ключем |
| `--version` | — | Показати версію |

Дайджест також показує блок «🔚 Згорають» — уже відомі тендери зі score ≥ 70,
у яких дедлайн ≤ 7 днів (щоб гарячий лід не згорів непоміченим).

### Health-моніторинг джерел

Кожен запуск пише статистику по джерелах у `.state.json → source_stats` і додає
в Telegram рядок виду `📡 TED 12 · DAB 3 · simap.ch 0`. Якщо джерело повертає
0 результатів 4+ запусків підряд — у повідомленні з'являється `⚠️` алерт:
HTML-розмітка порталу могла змінитись, перевір джерело
(`python tender_scraper.py --source <key> --days 30 --no-telegram`).

### Скоринг — налаштовується без коду

Ваги скорингу (CPV/keywords/бюджет/дедлайн) можна перевизначити в
`config.json → "scoring"`. Відсутні ключі беруться з дефолтів у коді.
...
```
tender-scraper/
├── config.json          # ← ключові слова, CPV коди, ваги скорингу (міняти для кожного клієнта)
├── .env.example         # ← шаблон токенів (cp .env.example .env)
├── tender_scraper.py    # головний скрапер (TED + bund.de + SIMAP + USP-AT + DE-портали)
├── tender_digest.py     # тижневий дайджест → Telegram (+ AI-аналіз за наявності ключа)
...
├── output/              # CSV виходи + outreach-чернетки (gitignored)
└── .state.json          # dedup стан + health-статистика джерел (gitignored)
```
